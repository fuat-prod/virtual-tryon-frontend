...
import { UserPlus, Mail, Lock, Loader2, CheckCircle2, ShoppingCart } from 'lucide-react';

export default function Register() {
  const navigate = useNavigate();
  const location = useLocation();
  const { registerWithEmail, migrateAnonymousToAuth, loginWithGoogle, loading, user } = useUser();

  // Ödeme akışından gelen parametreler
  const { returnUrl, selectedPlan, message } = location.state || {};
  const isFromPaywall = !!selectedPlan;
  ...
  const isAnonymousMigration = user && user.is_anonymous;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Eğer anonymous user varsa migration yap
      if (isAnonymousMigration) {
        result = await migrateAnonymousToAuth(formData.email, formData.password);
      } else {
        result = await registerWithEmail(formData.email, formData.password);
      }

      if (result.success) {
        // Eğer ödeme akışından geldiyse
        if (isFromPaywall && selectedPlan) {
          // TODO: Ödeme entegrasyonu tamamlandığında burası güncellenecek
          // initiatePaddleCheckout(selectedPlan);
          console.log('Payment flow: Plan selected ->', selectedPlan);
          
          // Şimdilik ana sayfaya yönlendir (ödeme entegrasyonu sonra)
          navigate(returnUrl || '/', { 
            state: { 
              message: 'Account created! Payment integration coming soon.',
              selectedPlan 
            }
          });
        } else {
          // Normal akış - ana sayfaya veya returnUrl'e dön
          navigate(returnUrl || '/');
        }
      } else {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!result.success) {
        setApiError(result.error || 'Google sign up failed');
      }
      // Google OAuth callback sonrası returnUrl'e yönlendirilecek
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 flex items-center justify-center p-4 pb-8 sm:p-6">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
            {isFromPaywall 
              ...
              : isAnonymousMigration 
                ? 'Save Your Progress' 
                : 'Create Account'}
          </h1>
          <p className="text-sm sm:text-base text-gray-600">
            {isFromPaywall
              ? message || 'Sign up to purchase credits'
              : isAnonymousMigration
                ? 'Register to keep your credits and generations'
                : 'Sign up to start your AI fashion journey'}
          </p>
        </div>

        {/* Payment Flow Info */}
        {isFromPaywall && selectedPlan && (
          <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
            <div className="flex items-start">
              <ShoppingCart className="w-5 h-5 text-purple-600 mr-3 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-sm font-medium text-purple-900">Selected Plan</p>
                <p className="text-sm text-purple-700 mt-1">
                  {selectedPlan.name} - ${selectedPlan.price}
                </p>
                <p className="text-xs text-purple-600 mt-1">
                  After registration, you'll complete your purchase
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Migration Info */}
        {isAnonymousMigration && user && !isFromPaywall && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start">
              <CheckCircle2 className="w-5 h-5 text-blue-600 mr-3 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-sm font-medium text-blue-900">Your progress will be saved!</p>
                <p className="text-sm text-blue-700 mt-1">
                  Credits: {user.credits} • Generations: {user.total_generations || 0}
                </p>
              </div>
            </div>
          </div>
